class MangaCopy extends ComicSource {
    ...
    version = "2.0.2"
    minAppVersion = "1.6.0"
    url = "https://cdn.jsdelivr.net/gh/istayheart/my-venera-newsources@latest/mangacopy_web_2_0_2.js"
    ...
    categoryComics = {
        load: async (category, param, options, page) => {
            ...
            let ordering = this.normalizeOrderingOption(options && options[0])
    ...
    search = {
        load: async (keyword, options, page) => {
            let qType = this.normalizeSearchOption(options && options[0])
            let limit = 12
    ...
    decryptWebPayload(value, keyText) {
        ...
        let plainText = this.stripJsonPadding(Convert.decodeUtf8(plainBytes))
        return JSON.parse(plainText)
    }

    stripJsonPadding(text) {
        text = String(text || "").trim()
        let start = text.search(/[\[{]/)
        if (start > 0) {
            text = text.substring(start)
        }
        let endObject = text.lastIndexOf("}")
        let endArray = text.lastIndexOf("]")
    ...
    normalizeSearchOption(value) {
        ...
        if (value === "" || value === "all" || value === "\"\"" || value === "''" || value === "null") {
            return ""
        }
        try {
            let parsed = JSON.parse(value)
            if (parsed == null || parsed === "" || parsed === "all") return ""
    ...
    normalizeOrderingOption(value) {
        if (value == null) return "-datetime_updated"
        value = String(value).trim()
        if (!value) return "-datetime_updated"
        let map = {
        ...
        let legacy = value.split("-")[0]
        return map[legacy] || "-datetime_updated"
    }
